...
internal class IOHelpers
{
    ...
    {
        if (!File.Exists(file)) File.WriteAllText(file, defaultContents);
    }
    ...
    public static void DeleteDirectoryIfExists(string directory, bool createDirectory = false)
    {
        if (Directory.Exists(directory))
        {
            ...
            if (!deleted) throw new Exception($"Could not delete directory: {directory}");
        }

        if (createDirectory) CreateDirectoryIfNotExists(directory);
    }
    ...
    public static void DeleteFileIfExists(string file, bool createFile = false)
    {
        ...
        if (createFile) CreateFileIfNotExists(file);
    }
    ...
    {
        if (overrideDirectory) DeleteDirectoryIfExists(outputDirectory, true);
        ...
        foreach (ZipEntry entry in zip)
        {
            ...
            if (!string.IsNullOrWhiteSpace(path))
            {
                ...
                {
                    if (excludedFiles.Contains(Path.GetFileName(entry.Name))) continue;

                    if (directoryPath is { Length: > 0 }) _ = Directory.CreateDirectory(directoryPath);
