...
internal class DotSlnTemplater : AbstractTemplater
{
    ...
    public override string GenerateProject(GenerateOptions options, Func<string, bool> log,
    ...
    protected override List<Property> ModifySolutionSettingProperties(string templateName, string outputDir,
        List<Property> properties, string solutionName)
    {
        ...
        var nugetFields = new List<string>()
            { nameof(DotSlnSolutionSettings.LicenseExpression), nameof(DotSlnSolutionSettings.Tags) };
        foreach (var property in properties)
        {
            // skip the license expression and tags if we don't ask for nuget info
            if (nugetFields.Contains(property.Name) && !templateSettings.AskForNugetInfo) continue;
            ...
            if (property.CurrentValue == null)
            {
                ...
                var defaultFromTemplateSettings =
                    templateSettings.GetType()?.GetField(defaultField)?.GetValue(templateSettings);
                ...
                else
                {
                    ...
                    actualDefault =
                        UpdateSimpleReplacementTextDuringSettingsRequest(actualDefault, outputDir, solutionName);
