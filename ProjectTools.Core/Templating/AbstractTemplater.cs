...
public abstract class AbstractTemplater
{
    ...
    public abstract string GenerateProject(GenerateOptions options, Func<string, bool> log,
        Func<string, bool> instructionLog, Func<string, bool> commandLog);
    ...
    public (List<Property> Properties, bool HadFile) GetGenerationSolutionSettingProperties(string templateName,
        string outputDir, string solutionConfigFile, string solutionName)
    {
        ...
        var properties = GetPropertiesForClass(SolutionSettingsType, typeof(SolutionSettingFieldMetadata), 0,
            PropertySource.SolutionSettings, existingSettings);
    ...
    public (List<Property> Properties, bool HadFile) GetPreperationUserSettings(string file)
    {
        ...
        var informationProperties = GetPropertiesForClass(TemplateInformationType, typeof(TemplateFieldMetadata), 0,
            PropertySource.TemplateInformation, existingTemplate?.Information);
        var settingsProperties = GetPropertiesForClass(TemplateSettingsType, typeof(TemplateFieldMetadata),
            informationProperties.Count, PropertySource.TemplateSettings, existingTemplate?.Settings);
    ...
    public (SolutionSettings, string) GetSolutionSettingsForProperties(List<Property> properties, string configName,
        bool preserveDefaultSolutionConfig)
    {
    ...
    public Template GetTemplateForProperties(List<Property> properties, string directory,
        bool createTemplateFile = true)
    {
        // Populate the child classes
        var informationProperties =
            properties.Where(x => x.SettingSource == PropertySource.TemplateInformation).ToList();
        var informationClass =
            PopulateNewInstanceWithProperties<TemplateInformation>(informationProperties, TemplateInformationType);

        var settingsProperties = properties.Where(x => x.SettingSource == PropertySource.TemplateSettings).ToList();
        var settingsClass =
            PopulateNewInstanceWithProperties<TemplateSettings>(settingsProperties, TemplateSettingsType);
    ...
    protected static T PopulateNewInstanceWithProperties<T>(List<Property> properties, Type actualType)
    {
        ...
        var instance = (T)(Activator.CreateInstance(actualType) ??
                           throw new Exception($"Unable to instantiate an instance of class {actualType}"));
        foreach (var property in properties)
        {
            var field = actualType.GetField(property.Name);
            if (field == null) throw new Exception($"Unable to find field {property.Name} in class {actualType}");
    ...
    protected List<Property> GetPropertiesForClass(Type type, Type attributeType, int orderOffset,
        PropertySource source, object? existingInstance = null)
    {
        var output = new List<Property>();

        // Fetch the properties for the class
        var fields = type.GetFields().Where(x => x.CustomAttributes.Any(y => y.AttributeType == attributeType))
            .ToList();
        foreach (var field in fields)
        {
            // get the metadata attribute
            var metadata = (TemplateFieldMetadata?)field.GetCustomAttributes(attributeType, false)[0];
            if (metadata == null) throw new Exception($"Field {field.Name} does not have a SettingMetadata Attribute!");

            // check if we have any allowed values
            List<string> allowedValues = [];
            var allowedValuesData =
                field.GetCustomAttributes(typeof(AllowedValue), false).Cast<AllowedValue>().ToList();
    ...
    protected abstract List<Property> ModifySolutionSettingProperties(string templateName, string outputDir,
        List<Property> properties, string solutionName);
    ...
    protected string UpdateSimpleReplacementTextDuringSettingsRequest(string value, string outputDir,
        string solutionName)
    {
        ...
        foreach (var replacer in replacers) value = value.Replace(replacer.Key, replacer.Value);
