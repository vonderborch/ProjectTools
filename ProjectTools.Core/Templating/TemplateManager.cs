...
public class TemplateManager
{
    ...
    public TemplateUpdateResult CheckForTemplateUpdates(bool forceUpdates = false, bool ignoreCache = false,
        bool autoUpdate = false)
    {
        ...
        else
        {
            ...
            var updateableTemplates = remoteTemplates.Where(x =>
                localTemplateNames.Contains(x.DisplayName) &&
                (GetTemplateByName(x.DisplayName)?.RepoInfo?.SHA != x.SHA || forceUpdates)).ToList();
            var orphanedTemplates =
                localTemplateNames.Where(x => !remoteTemplates.Any(y => y.DisplayName == x)).ToList();
            ...
            return new TemplateUpdateResult(remoteTemplates?.Count ?? -1, newTemplates, updateableTemplates,
                orphanedTemplates);
        }
    }
    ...
    public Implementation? DetectValidImplementationForDirectory(string directory)
    {
        foreach (var implementation in TemplaterFactory.GetImplementations())
        {
            ...
            if (templater.DirectoryValidForTemplatePreperation(directory)) return implementation;
        }
    ...
    public void DownloadTemplates(List<TemplateGitMetadata> templates)
    {
        ...
        var newTemplateCache = _localTemplates.Values.Cast<MetaTemplate>()
            .Where(x => !updatingNames.Contains(x.Template.Information.Name)).Select(x => x.RepoInfo).ToList();
        var updatingTemplates = _localTemplates.Values.Cast<MetaTemplate>()
            .Where(x => updatingNames.Contains(x.Template.Information.Name))
            .ToDictionary(x => x.Template.Information.Name, y => y);
    ...
    public void RefreshLocalTemplates()
    {
        ...
        var templatesFiles = Directory.GetFiles(Constants.TemplatesDirectory, $"*.{Constants.TemplateFileType}",
        ...
        foreach (var file in templatesFiles)
        {
            ...
            try
            {
                var templateCoreInfo =
                    JsonHelpers.DeserializeArchivedFile<Template>(file, Constants.TemplaterTemplatesInfoFileName);
                ...
                if (validTemplater == null) throw new Exception($"No valid templater found for template {fileName}");

                var template = new MetaTemplate()
                {
                    FilePath = file, RepoInfo = gitInfo, Template = templateCoreInfo,
                    Implementation = validTemplater.Implementation
                };
