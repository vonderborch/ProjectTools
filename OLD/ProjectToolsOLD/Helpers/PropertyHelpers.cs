    ...
    public static class PropertyHelpers
    {
        ...
        public static bool ContinueEditingSettings(List<Property> settings, Func<string, bool> LogMessage)
        {
            var displayedSettings = settings
                .Select(x => (x.Order,
                    $"{x.DisplayName}: {GetDisplayValue(x.CurrentValue, x.Type)}{Environment.NewLine}"))
            ...
            var result =
                ConsoleHelpers.GetYesNo($"Edit settings?{Environment.NewLine}{sb}{Environment.NewLine}", false);
            _ = LogMessage(" ");
            return result;
        }
        ...
        public static object GetInputForProperty(Property setting)
        {
            ...
            switch (setting.Type)
            {
                ...
                case PropertyType.String:
                    ...
                    {
                        response = ConsoleHelpers.GetInputWithLimit(setting.DisplayName, setting.AllowedValues,
                            defaultValue);
                    }
                    else
                    {
                ...
                case PropertyType.DictionaryStringString:
                    tempResponse = ConsoleHelpers.GetInput(setting.DisplayName, defaultValue);
                    tempListResponse = tempResponse.Split(",").Select(x => x.Trim()).ToList();
                    response = tempListResponse.Where(x => !string.IsNullOrWhiteSpace(x))
                ...
                case PropertyType.Enum:
                    ...
                    tempResponse =
                        ConsoleHelpers.GetInputWithLimit(setting.DisplayName, tempListResponse, defaultValue);
