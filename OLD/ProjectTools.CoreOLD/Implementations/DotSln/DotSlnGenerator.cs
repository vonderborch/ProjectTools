...
using ProjectTools.CoreOLD.Options;
...
namespace ProjectTools.CoreOLD.Implementations.DotSln
{
    ...
    {
        public DotSlnGenerator(Func<string, bool> log, Func<string, bool> instructionLog, Func<string, bool> commandLog) : base(log, instructionLog, commandLog)
        {
        }
        ...
        public override string GenerateProject(GenerateOptions options)
        {
            ...
            if (Directory.Exists(actualDirectory))
            {
                ...
                {
                    Logger.Log($"Directory {actualDirectory} already exists. Use the -f flag to force the generation of the solution.", 1);
                    return string.Empty;
                }
            }
            ...
            {
                var gitUrl = $"https://github.com/{options.SolutionSettings.GitRepoOwner}/{options.SolutionSettings.GitRepoName}.git";
            ...
            if (failedScripts.Count > 0)
            {
                ...
                {
                    Logger.Log($"Successfully executed {successfulScriptCount} script(s) and failed executing {failedScripts.Count} script(s)!", 1);
                }
                else
                {
            ...
            else
            {
                ...
                if (failedScripts.Count > 0)
                {
                    ...
                    {
                        _ = instructions.AppendLine($"- {script}");
                    }
            ...
            return $"Successfully prepared created the solution in {totalTime.TotalSeconds:0.00} second(s): {actualDirectory}";
        }
        ...
        private static void UpdateFiles(string directory, Dictionary<string, string> replacements, List<string> renameOnlyFilesAndDirectories)
        {
        ...
        {
            foreach ((var toBeReplaced, var replacedValue) in replacers)
            {
        ...
        private List<string> ExecuteScripts(List<string> scriptFiles, string actualDirectory)
        {
            ...
            for (var i = 0; i < scriptFiles.Count; i++)
            {
                ...
                {
                    CommandLogger.Log($"Script {i + 1}/{scriptFiles.Count}: {scriptFile} is not a Python script. Skipping...", 2);
                ...
                var updatedScriptContents = $"import os{Environment.NewLine}os.chdir(\"{actualWorkingDirectory}\"){Environment.NewLine}{scriptContents}";
        ...
        private Dictionary<string, string> GetReplacementText(GenerateOptions options, Template template)
        {
            ...
            // this after.
            foreach ((var key, var value) in template.Settings.ReplacementText)
            {
