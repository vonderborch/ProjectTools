...
public static class PropertyHelpers
{
    ...
    public static bool ContinueEditingSettings(List<Property> settings, Func<string, bool> LogMessage)
    {
        var displayedSettings = settings
            .Select(x => (x.Order, $"{x.DisplayName}: {GetDisplayValue(x.CurrentValue, x.Type)}{Environment.NewLine}"))
            .OrderBy(x => x.Order).ToList();

        var sb = new StringBuilder();
        foreach (var setting in displayedSettings) _ = sb.Append($"    {setting.Item2}");
