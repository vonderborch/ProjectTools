...
internal class UpdateTemplates : AbstractOption
{
    ...
    [Option('f', "force", Required = false, Default = false,
        HelpText = "If flag is provided, the program will force all templates to redownload.")]
    ...
    /// </summary>
    /// <value><c>true</c> if [force update]; otherwise, <c>false</c>.</value>
    [Option('i', "ignore-cache", Required = false, Default = true,
    ...
    public override string Execute()
    {
        ...
        _ = LogMessage(
            $"Discovered {updateResults.TotalRemoteTemplatesProcessed} remote template(s) in {totalSeconds} second(s)...");
        if (updateResults.NewTemplates.Count == 0 && updateResults.UpdateableTemplates.Count == 0 &&
            updateResults.OrphanedTemplates.Count == 0) return "No templates to update!";
        ...
        if (updateResults.NewTemplates.Count > 0)
        {
            ...
            if (Silent || ConsoleHelpers.GetYesNo(
                    $"Found {updateResults.NewTemplates.Count} new templates ({sizeAsMegabytes:0.000} megabyte(s))! Queue them for download?",
                    true))
            {
        ...
        if (updateResults.UpdateableTemplates.Count > 0)
        {
            ...
            if (Silent || ConsoleHelpers.GetYesNo(
                    $"Found {updateResults.UpdateableTemplates.Count} out-of-date templates ({sizeAsMegabytes:0.000} megabyte(s))! Queue them for download?",
                    true))
            {
        ...
        if (totalTemplatesToDownload.Count == 0) return "No templates queued for download!";

        // Download requested templates
        var totalSizeMb = totalDownloadingSize / 1024f / 1024f;
        _ = LogMessage(
            $"Downloading {totalTemplatesToDownload.Count} template(s) ({totalSizeMb:0.000} megabyte(s))...");
