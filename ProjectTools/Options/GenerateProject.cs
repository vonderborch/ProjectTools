...
internal class GenerateProject : AbstractOption
{
    ...
    [Option('p', "preserve-default-solution-config", Required = false, Default = false,
        HelpText =
    ...
    public override string Execute()
    {
        ...
        _ = LogMessage(
            $"Temple: {template.Template.Information.Name} (version {template.Template.Information.Version}) Project Generation");
        _ = LogMessage("----------------------------");
        ...
        if (WhatIf) return $"Solution not generated, but configuration settings saved: {SolutionConfig}";
    ...
    private SolutionSettings GetSolutionSettings(AbstractTemplater templater)
    {
        SolutionSettings settings;
        string backupFile;
        var solutionConfigFile = !string.IsNullOrEmpty(SolutionConfig)
            ? SolutionConfig
            : Constants.TemplatesProjectConfigurationFile;

        var (properties, hadFile) =
            templater.GetGenerationSolutionSettingProperties(Template, OutputDirectory, solutionConfigFile, Name);
        ...
        if (hadFile)
        {
            ...
            {
                (settings, backupFile) = templater.GetSolutionSettingsForProperties(properties, solutionConfigFile,
                    PreserveDefaultSolutionConfig);
        ...
        do
        {
            ...
            foreach (var setting in properties)
            {
                ...
                {
                    var otherField = properties.Where(x => x.Name == setting.Metadata.RequiredFieldName)
                        .FirstOrDefault();
                    ...
                    if (actualOtherValue != requiredOtherValue) continue;
                }
        ...
        (settings, backupFile) =
            templater.GetSolutionSettingsForProperties(properties, solutionConfigFile, PreserveDefaultSolutionConfig);
        _ = LogMessage($"  Configuration saved to: {backupFile}");
        return settings;
    }
}